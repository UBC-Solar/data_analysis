...
class Path:
    """
    reads the csv and interpolates the input data to produce linear lat/lon time series at the required time. the CSV consists of the columns time_s, lat, lon
    ...
    def __init__(self, csv_path: str):
        ...
        self.lat = df["lat"].values.astype(float)
        self.lon = df["lon"].values.astype(float)

        self.duration = self.t[-1]
        self.lat0 = self.lat[0]
        self.lon0 = self.lon[0]

        # convert to local E-N
        self.x, self.y = latlon_to_xy(self.lat, self.lon, self.lat0, self.lon0)
    ...
    def gps_interpolate(self, lat, lon, tstamp):
        ...
        data = [lat, lon]
        lat_interp = np.interp(tstamp, self.t, self.lat)
        lon_interp = np.interp(tstamp, self.t, self.lon)
        return lat_interp, lon_interp

    def latitude(self, t):
        # method to create a time series object of the latitude using gps interpolation
        return np.interp(t, self.t, self.lat)
    ...
    def vx(self, t):
        ...
        return np.interp(t, self.t, np.gradient(self.x, self.t))

    def vy(self, t):
        return np.interp(t, self.t, np.gradient(self.y, self.t))
    ...
    def ax(self, t):
        ...
        v = np.gradient(self.x, self.t)
        a = np.gradient(v, self.t)
        return np.interp(t, self.t, a)
...
class Sensor:
    ...
    def measure(self, path, t):
        """generate measurements for the certain time instance?"""
        return self.sample(path)[:t]
...
class GPSSensor(Sensor):
    ...
    def measure(self, path, t):
        return pd.DataFrame({"lat": path.latitude(t), "lon": path.longitude(t)})
...
class IMUSensor(Sensor):
    ...
    def _measure(self, path: Path, t: np.ndarray) -> pd.DataFrame:
        return pd.DataFrame({
            "ax": path.ax(t),
            "ay": path.ay(t),
